...
import com.land.base.oshi.SystemHardwareInfo;
import com.land.sys.core.constant.factory.ConstantFactory;
import com.land.sys.core.log.LogObjectHolder;
...
import com.land.sys.modular.system.service.NoticeService;
...
public class SystemController extends BaseController {
    ...
    @ResponseBody
    public ResponseData layuiUpload(@RequestPart("file") MultipartFile file,@RequestParam("businessKey") String businessKey) {

        UploadResult uploadResult = this.fileInfoService.uploadFile(businessKey,file);
     ...
     * 删除文件
     */
    @RequestMapping(method = RequestMethod.POST, path = "/deleteFiles")
    @ResponseBody
    public ResponseData deleteFiles(@RequestParam("ids") String ids) {

        boolean result = this.fileInfoService.deleteFiles(ids);
        HashMap<String, Object> map = new HashMap<>();

        return ResponseData.success(200, "删除成功", map);
    }

    @RequestMapping(method = RequestMethod.GET, path = "/downLoadFile")
    public ResponseData downLoadFile(@RequestParam("fileId") String fileId,HttpServletResponse response) {
        ...
        return ResponseData.success(200, "成功",response);
    }
